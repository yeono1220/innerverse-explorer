-- 모모의 해석(insight) 컬럼 + 시드 갱신. SQL Editor 에 붙여넣고 Run (경고 뜨면 Run query). 멱등.
...
begin
  ...
  insert into public.diary_entries
    (user_id, date, preview, body, audio_sec, emotions, keywords, primary_label, source, created_at, insight)
  values
    (uid, current_date - 1,
     ...
     '["수진","통화","보리","안부"]'::jsonb,
     '사랑', 'manual', now() - interval '1 day', '{"reason":"별 얘기 아니었다면서도 통화가 한 시간을 넘긴 걸 보면, 그 안부 한마디가 생각보다 큰 위로였던 것 같아.","reframe":"사람이 사람을 구한다는 말 — 오늘은 네가 구해진 날이지만, 언젠가는 네 전화가 수진이를 구하는 날도 올 거야.","next_step":"고마웠다고 짧게 한 줄 보내볼까? 답장은 기다리지 말고."}'::jsonb),
    (uid, current_date - 2,
     ...
     '긴장', 'voice', now() - interval '2 days', '{"reason":"손이 떨려 물을 세 번 마셨다는 대목에서 긴장이 제일 크게 읽혔어. 그런데 끝나고 안도와 뿌듯함이 같이 온 것도 분명해.","reframe":"떨림은 준비가 부족하다는 신호가 아니라, 이 일을 그만큼 소중히 여긴다는 신호일 수 있어.","next_step":"팀장님이 뭐라고 했는지 정확히 한 문장으로 적어둬. 다음 발표 전에 꺼내 보게."}'::jsonb),
    (uid, current_date - 3,
     ...
     '["비","산책","느림","마포"]'::jsonb,
     '차분', 'manual', now() - interval '3 days', '{"reason":"우산 없이 비를 맞으면서도 슬프지 않았다는 말에서, 요즘의 네 속도가 천천히로 맞춰진 게 느껴졌어.","reframe":"느린 게 뒤처지는 게 아니라, 네가 제일 잘 맞는 속도를 찾은 걸 수도 있어.","next_step":"오늘도 10분만 목적 없이 걸어볼까? 이어폰 없이."}'::jsonb),
    (uid, current_date - 4,
     ...
     '["야근","엄마","피로","미안함"]'::jsonb,
     '슬픔', 'manual', now() - interval '4 days', '{"reason":"엄마 전화를 두 번 놓치고 다시 걸 기운이 없었다는 데서 피로와 미안함이 겹쳐 보였어.","reframe":"전화를 못 받은 건 마음이 없어서가 아니라 마음이 다 써버려서야. 엄마도 아마 그걸 알 거야.","next_step":"지금 30초만 — 미안했다는 말보다 그냥 목소리 듣고 싶었다고 문자 하나."}'::jsonb),
    (uid, current_date - 5,
     ...
     '["러닝","한강","시작","상쾌"]'::jsonb,
     '기쁨', 'voice', now() - interval '5 days', '{"reason":"두 번 멈췄지만 끝까지 갔다는 문장이 이 일기의 중심이야. 다리는 아픈데 기분은 가볍다는 말이 그 증거고.","reframe":"미루던 걸 시작한 날은 결과보다 시작 자체가 성과야. 3km 는 덤이야.","next_step":"다음 러닝 날짜를 지금 달력에 찍어둬. 거리는 정하지 말고."}'::jsonb),
    (uid, current_date - 7,
     ...
     '["공허","일요일","정적"]'::jsonb,
     '공허', 'manual', now() - interval '7 days', '{"reason":"아무 일도 없는 날이 무섭다는 건, 바쁨이 사라지면 마음의 소리가 크게 들려서일 수 있어.","reframe":"텅 빈 하루도 기록해 두면 나중엔 쉼의 무늬로 보일지 몰라. 오늘 너는 그걸 이미 시작했어.","next_step":"보리 밥 준 것 말고 하나만 더 — 창문 열고 5분 바람 쐬기."}'::jsonb),
    (uid, current_date - 9,
     ...
     '["민재","마감","갈등","회의"]'::jsonb,
     '분노', 'manual', now() - interval '9 days', '{"reason":"틀린 말은 아닌데 방식이 싫었다는 데서, 화의 핵심이 내용이 아니라 회의에서 말해버린 방식이란 게 보여.","reframe":"열이 안 식는 건 네가 억울해서가 아니라, 그 프로젝트를 진짜 신경 쓰고 있어서일 수도 있어.","next_step":"민재한테 말 걸 필요 없어. 대신 그때 하고 싶었던 말을 여기 한 줄로 적어봐."}'::jsonb)
